...
class TechnicalIndicatorsDataSet(Dataset):
    ...
    def __init__(self, stocks_seq_size):
        ...
        indicators_to_normalize = ["stock_prices", "rsi", "macd", "macdignal", "macdhist", "sma", "ema", "upperband", "lowerband", "middleband"]
        ...
        print(self.company_indicators_combined)
            ...
            for company in company_names:
                if item["company_name"] == company:
                    for indicator in indicators_to_normalize:
                        data[f"{company}_{indicator}"].append(item["features"][f"{indicator}"][0])
        ...
        for key in self.company_indicators_combined:
            data = np.array(self.scalers[key]).reshape(-1, 1)
    ...
    def __getitem__(self, idx):
        ...
        indicators_to_normalize = ["rsi", "macd", "macdignal", "macdhist", "sma", "ema", "upperband", "lowerband", "middleband"]
        ...
        normalized_indicators = {
            key: self.scalers[f"{company_name}_{key}"].transform([[item["features"][key][0]]])[0][0]
